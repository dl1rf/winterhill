...
if [ $? == 0 ]; then
  lxterminal -t "whlaunch-local-3v20.sh" --working-directory=$HOME/winterhill/RPi-3v20/ -e ./whlaunch-local-3v20.sh
else
  grep -q 'BOOT = anyhub' winterhill/winterhill.ini
  if [ $? == 0 ]; then
    lxterminal -t "whlaunch-anyhub-3v20.sh" --working-directory=$HOME/winterhill/RPi-3v20/ -e ./whlaunch-anyhub-3v20.sh
  else
    grep -q 'BOOT = anywhere' winterhill/winterhill.ini
    if [ $? == 0 ]; then
      lxterminal -t "whlaunch-anywhere-3v20.sh" --working-directory=$HOME/winterhill/RPi-3v20/ -e ./whlaunch-anywhere-3v20.sh
    else
      grep -q 'BOOT = multihub' winterhill/winterhill.ini
      if [ $? == 0 ]; then
        lxterminal -t "whlaunch-multihub-3v20.sh" --working-directory=$HOME/winterhill/RPi-3v20/ -e ./whlaunch-multihub-3v20.sh
      else
        grep -q 'BOOT = fixed' winterhill/winterhill.ini
        if [ $? == 0 ]; then
          lxterminal -t "whlaunch-fixed-3v20.sh" --working-directory=$HOME/winterhill/RPi-3v20/ -e ./whlaunch-fixed-3v20.sh
        fi
      fi
    fi
  fi
