...
volatile    uint32_t            timpacks[256] ;
volatile    uint32_t            timsyncs[256] ;
volatile    uint8_t             tin ;
volatile    uint8_t             tout ;
volatile    uint32_t            lastin ;

            char                baseinterfaceaddress    [16] ;
            char                baseipaddress           [16] ;                  // the IP address   for all I/O
            uint16_t            baseipport ;                                    // the base IP port for all I/O
            char                buff                    [256] ;
            char                commandreplybuff        [512] ;
            char                commandrxbuff           [256] ;
            char                commandrxbuff2          [256] ;
            char                commandrxbuff3          [256] ;
const       uint32_t            daysinmonth             []   = {0,31,28,31,30,31,30,31,31,30,31,30,31} ;
            char                expandedtextinfo        [65536] ;
            char                EITDEFAULTS             [64] = {13,12,6,9,12,18,0} ;
            struct eitx         eit ;                   // EIT packet
            uint32_t            eitinsert ;             // an EIT packet with info is inserted into the TS
            uint32_t            eitremove ;             // EIT packets are removed from the incoming TS
            uint32_t            GLOBALNIM ;
volatile    int32_t             lminfoutenabled ;       // enable info sending
volatile    uint32_t            i2csharedaccess ;       // controls i2c access
            pthread_t           info_thread ;
            uint32_t            lastinfotime ;          // time of last info transmission (ms)
            char                logfilename [64] ;      // name of the log file
            uint32_t            h265max ;               // maximum number of VLC windows to use the hardware decoder
            uint32_t            idletime ;              // receiver is disabled after this many seconds of inactivity
            uint32_t            inicommandcount ;       // number of commands in the ini file
volatile    int32_t             inicommandenabled ;     // enable ini command sending
            ...
            uint32_t            modex ;                 // program and TS distribution mode
            uint32_t            null8190 ;              // PID 8190 is treated as a NULL packet
            uint32_t            nullremove ;            // NULL packets are not sent to VLC
            uint32_t            offnettime ;            // off net transmission is stopped after this many seconds
                                                        // of no commands when sending off net
            uint32_t            peripherals_virtual_address ;   // virtual address of the peripherals
            uint32_t            rxbase ;                // the 4 receivers are numbered starting at this value
volatile    uint32_t            terminate ;
            pthread_t           tsproc_thread ;
volatile    uint32_t            txpbindexin ;           // indexes for the UDP sending ring buffer
volatile    uint32_t            txpbindexout ;
volatile    int32_t             tsprocenabled ;         // enable UDP packet sending
            int32_t             qo100beaconfreq ;       // nominal frequency of the beacon
            uint32_t            vgxtone ;               // voltage generator X tone
            uint32_t            vgxen ;                 // voltage generator X enable               --> bit0
            uint32_t            vgxsel ;                // voltage generator X low / high select    --> bit2
            uint32_t            vgytone ;               // voltage generator Y tone
            uint32_t            vgyen ;                 // voltage generator Y enable               --> bit4
            uint32_t            vgysel ;                // voltage generator Y low / high select    --> bit6
            int                 whfd ;
            char                nullpacket [188] ;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            ...
            uint32_t        calculateCRC32              (uint8_t*, uint32_t) ;
            int32_t         configsockets               (uint32_t, uint32_t) ;
            void*           info_loop                   (void*) ;
            void            getdatetime                 (char*) ;
            int32_t         getiptype                   (char*) ;
            void*           inicommand_loop             (void*) ;
            void            juliandate                  (int32_t*, int32_t*) ;
            void            logit                       (char*) ;
            uint32_t        monotime_ms                 (void) ;
            int32_t         opensocket                  (uint32_t, char*, uint16_t*, uint32_t, int*, struct sockaddr_in*) ;
            uint32_t        reverse                     (uint32_t) ;
            void            setup_eit                   (void*, uint32_t, char*) ;
            int             setup_io_map                (void) ;
            void            setup_titlebar              (char*, uint32_t) ;
    ...
    else
    {
        ...
        while (!feof(ip))
        {
            ...
            do
            {
                ...
                if (pos)
                {
                    memcpy(pos,pos+1,strlen(pos+1)+1) ;
                }
            } while (pos) ;
...
// write spi5interruptnumber to the driver
    ...
    *(uint32_t*)buff = spi5interruptnumber ;
    write (fd, buff, 4) ;
...
// close and re-open the driver
    ...
    close (whfd) ;
    whfd = open (temps, O_RDWR) ;
    *(uint32_t*)buff = spi5interruptnumber ;            // do it again for good measure
    write (fd, buff, 4) ;
    ...
    if (rx > 0 && freqx >= 0 && locx >= 0 && srx > 0)
    {
        ...
        if (((abs(freqx - locx) >= MINFREQ) && (abs(freqx - locx) <= MAXFREQ)) || freqx == 0)
        {
            if ((srx >= MINSR) && (srx <= MAXSR))
            {
                ...
                if (rcv[rx].receiver)                                       // see if receiver exists
                {
                    int32_t antx = rcv[rx].rawinfos[STATUS_ANTENNA];
...
int32_t configsockets (uint32_t receiver, uint32_t offon)
{
    int32_t     err ;
    int         rx ;
...
int32_t opensocket (uint32_t offon, char* address, uint16_t* port, uint32_t listening, int* sock, struct sockaddr_in* sockaddr)
{
    int                 status ;
    struct ip_mreq      mreq ;
...
void* tsproc_loop (void* dummy)
{
        ...
        uint32_t    audioservicetype ;
        uint32_t    videoservicetype ;
        uint32_t    prognumber ;
        uint32_t    pmtpid ;
        uint32_t    programcount ;
        uint32_t    changedetected ;
        uint8_t     rxbuff [256] ;
...
// select the first PMT with program number > 0
                    ...
                    if (pid == PAT_PID)
                    {
                        ...
                        if (sectionlength)
                        {
                            tempu = calculateCRC32 ((uint8_t*)&pp->data[index],sectionlength-1) ;
                            tempu = reverse (tempu) ;                                   // reverse byte order
                            index += sectionlength - 1 ;
                            if (tempu == *((uint32_t*)&pp->data[index]))                    // CRC32 OK
                            {
                                pmtpid = 0 ;                                            // PMT to use
...
// look for service types
                    ...
                    else if (pid == SDT_PID)                                // service descriptor
                    {
                        ...
                        if (sectionlength)
                        {
                            ...
                            tempu = reverse (tempu) ;                           // reverse byte order
                            index += sectionlength - 1 ;
                            if (tempu == *((uint32_t*)&pp->data[index]))
                            {
                                sdtp = (uint8_t*) pp ;
...
void setup_titlebar (char *output, uint32_t rx)
{
    int             x ;
    int             y ;
    char            temps  [512] ;
...
// display TS type and video type
        ...
        if (rcv[rx].scanstate == STATE_DEMOD_S2 || rcv[rx].scanstate == STATE_DEMOD_S)
        {
            ...
            if (strlen(temps))
            {
                ...
                {
                    memcpy(temps,temps+4,strlen(temps+4)+1) ;       // S or S2
                }
                sprintf (output+strlen(output), "%s", temps) ;
            }
...
// display modulation and FEC
        ...
        if (rcv[rx].scanstate == STATE_DEMOD_S2 || rcv[rx].scanstate == STATE_DEMOD_S)
        {
            ...
            if (pos)
            {
                memcpy(pos+2,pos+5,strlen(pos+5)+1) ;
            }
            else
            {
                ...
                else
                {
                    ...
                    if (pos)
                    {
                        memcpy(pos+1,pos+5,strlen(pos+5)+1) ;
                    }
                }
            }
            sprintf (output+strlen(output)," %s",temps) ;
...
void setup_eit (void *packet, uint32_t rx, char* info)
{
    int             y ;
    uint8_t         *p, *q ;
    uint            utempi ;
    ...
    eitp->servicehigh   = (uint8_t) (rcv[rx].serviceid >> 8) ;
    eitp->servicelow    = (uint8_t) (rcv[rx].serviceid & 0xff) ;
    ...
    eitp->tsidhigh      = (uint8_t) (TSID >> 8) ;               // TSID = 0; not needed by VLC for EIT
    eitp->tsidlow       = (uint8_t) (TSID & 0xff) ;
    eitp->networkhigh   = (uint8_t) (NETWORK >> 8) ;            // NETWORK = 0; not needed by VLC for EIT
    eitp->networklow    = (uint8_t) (NETWORK & 0xff) ;
    eitp->lasttable     = 0x4e ;                            // service descriptor

    p = (uint8_t*) &eitp->loop ;

    *p++ = (uint8_t) (EVENTID >> 8) ;                       // event id = zero (first event)
    *p++ = (uint8_t) (EVENTID & 0xff) ;
    ...
    *q = (uint8_t) (p - q - 1) ;
    ...
    eitp->sectionlength = (uint8_t) ((uint32_t)(uintptr_t)p - (uint)(uintptr_t)&eitp->sectionlength - 1 + 4) ;

    utempi = calculateCRC32 ((uint8_t*)&eitp->tableid,eitp->sectionlength-1) ;
    *p++ = (uint8_t) ((utempi >> 24) & 0xff) ;
    *p++ = (uint8_t) ((utempi >> 16) & 0xff) ;
    *p++ = (uint8_t) ((utempi >>  8) & 0xff) ;
    *p++ = (uint8_t) ((utempi >>  0) & 0xff) ;

    memset (p, 0xff, 188-((uint)(uintptr_t)p-(uint)(uintptr_t)eitp)) ;  // pad out the rest of the packet
}
...
void setup_eit2 (void *packet, uint32_t rx, char* info)
{
    int             y ;
    uint8_t         *p, *q ;
    uint            utempi ;
...
void juliandate (int32_t* julday, int32_t* julhour)
{
    uint16_t        utempi ;
    int             x ;
    int             year ;
...
uint32_t calculateCRC32 (uint8_t* data, uint32_t dataLength)
{
    uint32_t    crc ;
    uint32_t    poly, temp, temp2, temp4, bit31 ;
    uint32_t    x ;
...
uint32_t reverse (uint32_t indata)
{
    uint32_t    tempu ;
