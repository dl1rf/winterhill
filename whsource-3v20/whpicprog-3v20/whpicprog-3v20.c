...
#define ERROR_WHRUNNING		0xfffffff6
...
typedef struct
{
	uint32_t	active;									// active or not
	uint32_t	startindex;								// line index of the start point
	uint32_t	iterations	;							// loop count (descending)
} progloop_t;
...
uint32_t	address;
uint8_t		flashbytes[FLASHBYTES];
uint8_t		flashbytes2[FLASHBYTES];
uint32_t*	gpio;								// pointer to the GPIO area
...
uint32_t	pclock;
uint32_t	pdata;
uint32_t	peripherals_base;					// base of all peripherals; 0xfe0000 for RPi4
uint32_t	preset;
...
uint32_t	check_pic_status	( uint32_t );
int32_t		i2c_read_reg8		( uint8_t, uint8_t, uint8_t* );
int32_t		i2c_write_reg8		( uint8_t, uint8_t, uint8_t );
uint32_t	get_return_value	( uint32_t );
void		gpioconfig			( uint32_t, uint32_t );
uint32_t	gpioget				( uint32_t );
void		gpioset				( uint32_t, uint32_t );
void		icsp				( progline_t*, uint32_t*, uint32_t, uint32_t, uint32_t );
//void		pdelay				( void );
uint32_t	receivebits			( uint32_t );
void		restore_pins		( void );
void		sendbits			( uint32_t, uint32_t );
uint32_t*	setup_io_map		( uint32_t );
void		setup_pins			( void );
pid_t		find_proc			( const char* name );
...
int32_t main ( int argc, char* argv[] )
{
	...
	char		hexline[256];
	FILE*		ip;
	uint32_t	parsedbytecount;
	uint8_t		parsedbytes[128];
	uint32_t	picaddress;
	char*		pos;
	uint32_t	recordtype;
	uint32_t*	rpiaddress;
	uint32_t*	rpiaddress2;
	int			temp1;
	int			temp2;
	uint32_t	tempu;
	uint32_t	x;
	...
	printf ("\r\n");
	printf ("------------------------------------------------------------------------\r\n");
	printf ("%s: PIC24FJ256GA702 in-circuit programmer\r\n",VERSION);
	printf ("                for the BATC Advanced Receiver PCB\r\n");
	printf ("------------------------------------------------------------------------\r\n");
...
// check we are being run as sudo
	...
	if (geteuid() != 0)
	{
		...
		printf ("Usage: sudo ./%s hexfilename\r\n",VERSION) ;
		printf ("       (will program both PICs using the hex file)\r\n") ;
		printf ("Usage: sudo ./%s --check\r\n",VERSION) ;
		printf ("       (check for PIC presence and software version)\r\n") ;
	    printf ("------------------------------------------------------------------------\r\n") ;
		printf ("\r\n") ;
...
// check for running winterhill main program

	if (-1 != find_proc("./winterhill-3v20"))
	{
		printf ("Running WinterHill program detected.\r\n");
		printf ("Please stop the WinterHill program and try again.\r\n");
		printf ("------------------------------------------------------------------------\r\n");
		printf ("\r\n");
		usleep (2 * 1000 * 1000);							// 2s
		exit (ERROR_WHRUNNING);
	}
...
// look for the I2C port
	...
	if (i2cfd < 1)
	{
		printf ("Cannot open /dev/i2c-1\r\n");
		printf ("has the I2C port been enabled in RPi Configuration?\r\n");
		printf ("------------------------------------------------------------------------\r\n");
		printf ("\r\n");
		restore_pins();
		usleep (2 * 1000 * 1000);							// 2s
...
// initialise the buffers for PIC writing and comparing
	...
	rpiaddress  = (uint32_t*)(uintptr_t)flashbytes ;
	rpiaddress2 = (uint32_t*)(uintptr_t)flashbytes2;
...
// exit icsp mode and re-enter; programming fails every alternate time if this is not done
		...
		for (x = 0; x < PAGES; x++)
		{
			if (usedpages[x])
			{
				...
				rpiaddress = (uint32_t*)(uintptr_t)((uint64_t)x * (uint64_t)PAGEBYTES + (uint64_t)(uintptr_t)(&flashbytes));
		...
		for (x = 0; x < PAGES; x++)
		{
			if (usedpages[x])
			{
				...
				rpiaddress  = (uint32_t*)(uintptr_t)((uint64_t)x * (uint64_t)PAGEBYTES + (uint64_t)(uintptr_t)&flashbytes);
				rpiaddress2 = (uint32_t*)(uintptr_t)((uint64_t)x * (uint64_t)PAGEBYTES + (uint64_t)(uintptr_t)&flashbytes2);
				picaddress  = x * PAGEBYTES / 2;                      	// 16 bit words
...
void sendbits ( uint32_t value, uint32_t bitcount )
{
	unsigned	x;
...
uint32_t receivebits ( uint32_t bitcount )
{
	unsigned	x;
	uint32_t	temp;
...
void icsp ( progline_t* icspsequence, uint32_t* xrpiaddress, uint32_t xpicaddress, uint32_t xcount1, uint32_t xcount2 )
{
	int			x;
	uint32_t	outvalue;
	uint32_t	visilow;
...
void gpioconfig (uint32_t bcmportno, uint32_t altfunction)
{
	uint32_t                    index;
	uint32_t                    pos;
	uint32_t                    temp;
...
uint32_t gpioget (uint32_t bcmbitno)
{
	uint32_t		temp;
...
void gpioset (uint32_t bcmbitno,uint32_t value)
{
	if (value)
	{
...
unsigned* setup_io_map (unsigned offset)									// return a pointer to the required peripheral offset
{
	int			mem_fd;
	void		*peri_map;
	unsigned	io_base;
	unsigned	bcm_peri_base;
...
/* mmap peripheral */
	...
	if ((int)(intptr_t)peri_map == -1)
	{
	   printf("mmap error %d \r\n", (int)(intptr_t)peri_map);	// errno also set!
	   return (0);
	}

	return (unsigned *) peri_map;
}
...
int32_t i2c_read_reg8 (uint8_t i2caddr, uint8_t reg, uint8_t *val)
{
///    uint8_t err;
...
uint32_t check_pic_status (uint32_t returnstatus)
{
	int32_t		status;
	uint32_t	picid[2];
	uint32_t	picversion;
...
// get PIC_A version
	...
	{
		picversion = 0;
		status  = i2c_read_reg8 (PIC_ADDR_A, PIC_REG_VERSION_HIGH, (uint8_t*)&picversion);
		picversion <<= 8;
		status |= i2c_read_reg8 (PIC_ADDR_A, PIC_REG_VERSION_LOW,  (uint8_t*)&picversion);
		if (status < 0)
		{
...
// get PIC_B version
	...
	{
		picversion = 0;
		status  = i2c_read_reg8 (PIC_ADDR_B, PIC_REG_VERSION_HIGH, (uint8_t*)&picversion);
		picversion <<= 8;
		status |= i2c_read_reg8 (PIC_ADDR_B, PIC_REG_VERSION_LOW,  (uint8_t*)&picversion);
		if (status < 0)
		{
...
uint32_t get_return_value (uint32_t retstat)
{
	unsigned	x;
	uint32_t		retval;
	uint32_t		tempu;
